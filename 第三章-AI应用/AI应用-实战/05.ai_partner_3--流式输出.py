...
# 设置页面的配置项
st.set_page_config(
    page_title="AI智能伴侣",  #浏览器最上面的小页面显示的
...
if prompt:                                        #字符串会自动转化为布尔值,如果字符串非空,则为True,否则为False
    ...
    # ==========改动节点1：接口开启流式开关（非流式→流式第一处核心修改）==========
    # 非流式：stream=False，一次性接收完整回复；流式：stream=True，分片逐段接收数据
    ...
    # ==========改动节点2：新增流式专属容器+循环分片解析（最大改动点）==========
    # 非流式无需循环，直接取最终值；流式要循环遍历分片chunk、拼接增量文字
    response_message = st.empty()                           # 创建空容器：专门用来动态刷新流式文字，非流式不需要该容器
    full_response = ""                                      # 全局字符串：累加所有分片内容，最后存入会话记忆，非流式可直接用接口返回值

    # 循环遍历每一段流式返回分片chunk（流式独有逻辑，非流式无循环）
    for chunk in response:
        # 过滤空分片，避免拼接无效内容
        if chunk.choices[0].delta.content is not None:
            content = chunk.choices[0].delta.content
            full_response += content                        # full_response初始为空字符串，增量拼接新片段
            response_message.chat_message("assistant").write(full_response)  # 局部刷新气泡内文字，实现打字机效果

    # 保存大模型返回的结果--AI回答入库（流式/非流式共用收尾逻辑，无需改动）
